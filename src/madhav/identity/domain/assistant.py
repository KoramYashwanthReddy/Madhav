...
from madhav.version import VERSION


class AssistantIdentity(BaseModel):
    """Formal identity representation of the MADHAV assistant itself."""

    id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="Unique assistant identifier"
    )
    name: str = Field(default="Madhav", description="Canonical assistant name")
    display_name: str = Field(default="MADHAV Personal AI", description="User-facing display name")
