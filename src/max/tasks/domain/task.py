...
class TaskResult(BaseModel):
    ...
    output_reference: str | None = Field(default=None, description="Pointer to generated output artifact")
...
class TaskFailure(BaseModel):
    ...
    occurred_at: datetime = Field(default_factory=datetime.utcnow, description="Failure occurrence timestamp")
...
class Task(BaseModel):
    ...
    id: str = Field(default_factory=lambda: f"task_{uuid4().hex[:12]}", description="Unique task identifier")
    ...
    priority: TaskPriority = Field(default=TaskPriority.NORMAL, description="Controlled priority level")
    ...
    plan_id: str | None = Field(default=None, description="Linked Module 11 plan ID if derived from plan")
    ...
    parent_task_id: str | None = Field(default=None, description="Parent task ID for hierarchical subtasks")
    ...
    started_at: datetime | None = Field(default=None, description="Timestamp when task transitioned to IN_PROGRESS")
    completed_at: datetime | None = Field(default=None, description="Timestamp when task reached COMPLETED")
    cancelled_at: datetime | None = Field(default=None, description="Timestamp when task was CANCELLED")

    result: TaskResult | None = Field(default=None, description="Outcome result if task completed")
    failure: TaskFailure | None = Field(default=None, description="Failure diagnostic if task failed")
    ...
    last_retry_at: datetime | None = Field(default=None, description="Timestamp of last retry reset")
    retryable: bool = Field(default=True, description="Flag indicating if task can be retried upon failure")
